...
from cartoonFrame import CartoonFrame
...
class EditBar(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.cartoon_button = Button(self, text="Cartoonify")
        self.border_button = Button(self, text="Border")
        self.denoise_button = Button(self, text="Remove Noise")
        self.bgremove_button = Button(self, text="Remove Background")
        ...
        self.addtext_button.bind("<ButtonRelease>", self.addtext_button_released)
        ...
        self.denoise_button.pack(side=LEFT)
        self.bgremove_button.pack(side=LEFT)
    ...
    def cartoon_button_released(self, event):
        if self.winfo_containing(event.x_root, event.y_root) == self.cartoon_button:
            if self.master.is_image_selected:
                ...
                self.master.cartoonFrame = CartoonFrame(master=self.master)
                self.master.cartoonFrame.grab_set()
    ...
    def border_button_released(self, event):
        if self.winfo_containing(event.x_root, event.y_root) == self.border_button:
            if self.master.is_image_selected:
                ...
                self.master.borderFrame = BorderFrame(master=self.master)
                self.master.borderFrame.grab_set()

    def denoise_button_released(self, event):
        if self.winfo_containing(event.x_root, event.y_root) == self.denoise_button:
            if self.master.is_image_selected:
                if self.master.is_draw_state:
                    self.master.image_viewer.deactivate_draw()
                if self.master.is_crop_state:
                ...
                self.master.deNoise = DeNoise(master=self.master)
                self.master.deNoise.grab_set()

    def bgremove_button_released(self, event):
        if self.winfo_containing(event.x_root, event.y_root) == self.bgremove_button:
            if self.master.is_image_selected:
                if self.master.is_draw_state:
                    self.master.image_viewer.deactivate_draw()
                if self.master.is_crop_state:
                ...
                self.master.bgRemove = BgRemove(master=self.master)
                self.master.bgRemove.grab_set()

    def addtext_button_released(self, event):
        if self.winfo_containing(event.x_root, event.y_root) == self.addtext_button:
            if self.master.is_image_selected:
                if self.master.is_draw_state:
                    self.master.image_viewer.deactivate_draw()
                if self.master.is_crop_state:
                ...
                self.master.textEdit = TextEdit(master=self.master)
                self.master.textEdit.grab_set()
